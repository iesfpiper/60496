...
def sliceData(low, high, exclude=None, numRandom=-1):
    dataArr=sampleData[(((((sampleMeta[:,0]-1)*10 + sampleMeta[:,1]) >=low) & (((sampleMeta[:,0]-1)*10 + sampleMeta[:,1]) <=high)) & (sampleClass[:] != exclude))]
    classArr=sampleClass[(((((sampleMeta[:,0]-1)*10 + sampleMeta[:,1]) >=low) & (((sampleMeta[:,0]-1)*10 + sampleMeta[:,1]) <=high)) & (sampleClass[:] != exclude))]
    if numRandom>0:
...
DATASET_PATH = [PROJECT_ROOT + '\\datasets\\DSL-StrongPasswordDatafirst10.csv']

samplesPerSubject = 80

#**setup output
f=open(PROJECT_ROOT+"\\output\\output10.txt","w+")
...
for initialSamplesSize in range(1,11,1):
    samplesPerTrial=10
    ...
    for k in range(1,10,2):
        ...
        for i in range(initialSamplesSize+1,71,samplesPerTrial):
